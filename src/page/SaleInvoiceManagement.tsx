...
interface InvoiceActionsMenuProps {
  ...
  onDownloadPDF: (id: string, invoiceNumber: string) => void // ✅ Added for Sale
  isSending: boolean
...
const SaleInvoiceManagement = () => {
  usePageTitle('Danh sách hóa đơn - Sale')
  ...
  const loadInvoices = async () => {
    try {
      ...
      if (filteredInvoicesData.length === 0) {
        console.warn('⚠️ [SaleInvoiceManagement] No invoices found for salesID:', currentSalesID)
      }
      ...
      // Map only filtered invoices (đã filter theo salesID rồi)
      let mappedData = filteredInvoicesData.map(item => mapInvoiceToUI(item, templateMap, customerMap))
