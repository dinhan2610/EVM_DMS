...
import type { CreateTemplateApiRequest, UpdateTemplateApiRequest, TemplateApiResponse } from '@/types/templateApi'
...
export const getAllPrefixes = async (): Promise<PrefixResponse[]> => {
  try {
    const response = await axios.get<PrefixResponse[]>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.INVOICE_SYMBOL.PREFIX}`, {
...
export const getAllSerialStatuses = async (): Promise<SerialStatusResponse[]> => {
  try {
    const response = await axios.get<SerialStatusResponse[]>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.INVOICE_SYMBOL.SERIAL_STATUS}`, {
      headers: getAuthHeaders(),
    })
...
export const getAllInvoiceTypes = async (): Promise<InvoiceTypeResponse[]> => {
  try {
    const response = await axios.get<InvoiceTypeResponse[]>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.INVOICE_SYMBOL.INVOICE_TYPE}`, {
      headers: getAuthHeaders(),
    })
...
export const getAllSerials = async (): Promise<SerialResponse[]> => {
  try {
    ...
    const response = await axios.get<SerialResponse[]>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.SERIAL.GET_ALL}`, { headers: getAuthHeaders() })
...
export const createSerial = async (data: CreateSerialRequest): Promise<SerialResponse> => {
  try {
    ...
    const response = await axios.post<SerialResponse>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.SERIAL.CREATE}`, data, {
      headers: getAuthHeaders(),
    })
 ...
 */
export const getTemplatePreviewHtml = async (templateId: number): Promise<string> => {
  try {
    console.log('[getTemplatePreviewHtml] Fetching preview for template:', templateId)

    const response = await axios.get<string>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.TEMPLATE.PREVIEW_HTML(templateId)}`, {
      headers: getAuthHeaders(),
 ...
 */
export const getTemplatePreviewPdf = async (templateId: number): Promise<Blob> => {
  try {
    console.log('[getTemplatePreviewPdf] Fetching PDF preview for template:', templateId)

    const response = await axios.get(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.TEMPLATE.PREVIEW_HTML(templateId)}`, {
      headers: getAuthHeaders(),
...
export const createTemplate = async (data: CreateTemplateRequest): Promise<TemplateResponse> => {
  try {
    ...
    const response = await axios.post<TemplateResponse>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.TEMPLATE.CREATE}`, requestData, {
      headers: getAuthHeaders(),
    })
...
export const getAllTemplates = async (): Promise<TemplateResponse[]> => {
  try {
    const response = await axios.get<TemplateResponse[]>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.TEMPLATE.GET_ALL}`, {
      headers: getAuthHeaders(),
    })
...
export const getTemplateById = async (templateId: number): Promise<TemplateResponse> => {
  try {
    ...
    const response = await axios.get<TemplateResponse>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.TEMPLATE.GET_BY_ID(templateId)}`, {
      headers: getAuthHeaders(),
    })
    ...
    console.log('[getTemplateById] ✅ Success:', {
      ...
      layoutDefinitionPreview:
        typeof response.data.layoutDefinition === 'string' ? response.data.layoutDefinition.substring(0, 200) : response.data.layoutDefinition,
    })
 ...
 * })
 */
export const updateTemplate = async (templateId: number, data: UpdateTemplateRequest): Promise<TemplateResponse> => {
  try {
    ...
    const response = await axios.put<TemplateResponse>(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.TEMPLATE.UPDATE(templateId)}`, data, {
      headers: getAuthHeaders(),
    })
...
export const uploadLogo = async (file: File): Promise<string> => {
  try {
    ...
    const response = await axios.post<{ url: string }>(`${API_CONFIG.BASE_URL}/File/upload-template-image`, formData, {
      params: {
